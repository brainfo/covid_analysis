...
output_braken_file="${outdir}/bracken_${acc_num}_results.tab"
report_braken_file="${outdir}/bracken_${acc_num}_reports.tab"
#################################################################
# Start work
if [ ! -f output_file ]; then
    srun --job-name=${acc_num}_kraken kraken2 --threads ${SLURM_CPUS_PER_TASK} --gzip-compressed --paired --db ${db} ${query_fa} --output ${output_file} --report ${report_file}
fi
echo "srun --job-name=${acc_num}_bracken bracken -d ${db} -i ${report_file} -o ${output_braken_file} -w ${report_braken_file} -r 50 -t 5"
srun --job-name=${acc_num}_bracken bracken -d ${db} -i ${report_file} -o ${output_braken_file} -w ${report_braken_file} -r 50 -t 5
#################################################################
